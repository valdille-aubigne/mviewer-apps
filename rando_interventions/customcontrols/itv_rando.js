mviewer.customControls.itv_rando = (function() {
    ...
    return {
		...
		{mviewer.customControls.itv_rando.send_comment(text.value,_xy[0],_xy[1],date_observation.value,mail.value,newFile, nom.value);}
		else
		{mviewer.alert(erreur,"alert-info");}
		 },

		send_comment: function (comment,coord_x,coord_y,date_observation,mail,image,nom)
		{
		...
		xhr.send("comment="+scomment+"&coord_x="+coord_x+"&coord_y="+coord_y+"&date_observation="+date_observation+"&mail="+mail+"&imagename="+imagename+"&nom="+nom);
